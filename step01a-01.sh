...
head -n 82 "$RAW_FILE_LIST_FILE" | tail -n +2 | cut -f 1 > "$WD"/output/url-links_folder_01_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 82 "$RAW_FILE_LIST_FILE" | tail -n +2 | cut -f 3 > "$WD"/output/url-links_folder_01_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 134 "$RAW_FILE_LIST_FILE" | tail -n +83 | cut -f 1 > "$WD"/output/url-links_folder_02_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 134 "$RAW_FILE_LIST_FILE" | tail -n +83 | cut -f 3 > "$WD"/output/url-links_folder_02_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 214 "$RAW_FILE_LIST_FILE" | tail -n +135 | cut -f 1 > "$WD"/output/url-links_folder_03_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 214 "$RAW_FILE_LIST_FILE" | tail -n +135 | cut -f 3 > "$WD"/output/url-links_folder_03_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 294 "$RAW_FILE_LIST_FILE" | tail -n +215 | cut -f 1 > "$WD"/output/url-links_folder_04_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 294 "$RAW_FILE_LIST_FILE" | tail -n +215 | cut -f 3 > "$WD"/output/url-links_folder_04_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 376 "$RAW_FILE_LIST_FILE" | tail -n +295 | cut -f 1 > "$WD"/output/url-links_folder_05_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 376 "$RAW_FILE_LIST_FILE" | tail -n +295 | cut -f 3 > "$WD"/output/url-links_folder_05_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 456 "$RAW_FILE_LIST_FILE" | tail -n +377 | cut -f 1 > "$WD"/output/url-links_folder_06_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 456 "$RAW_FILE_LIST_FILE" | tail -n +377 | cut -f 3 > "$WD"/output/url-links_folder_06_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 536 "$RAW_FILE_LIST_FILE" | tail -n +457 | cut -f 1 > "$WD"/output/url-links_folder_07_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 536 "$RAW_FILE_LIST_FILE" | tail -n +457 | cut -f 3 > "$WD"/output/url-links_folder_07_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 616 "$RAW_FILE_LIST_FILE" | tail -n +537 | cut -f 1 > "$WD"/output/url-links_folder_08_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 616 "$RAW_FILE_LIST_FILE" | tail -n +537 | cut -f 3 > "$WD"/output/url-links_folder_08_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 696 "$RAW_FILE_LIST_FILE" | tail -n +617 | cut -f 1 > "$WD"/output/url-links_folder_09_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 696 "$RAW_FILE_LIST_FILE" | tail -n +617 | cut -f 3 > "$WD"/output/url-links_folder_09_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 776 "$RAW_FILE_LIST_FILE" | tail -n +697 | cut -f 1 > "$WD"/output/url-links_folder_10_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 776 "$RAW_FILE_LIST_FILE" | tail -n +697 | cut -f 3 > "$WD"/output/url-links_folder_10_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 856 "$RAW_FILE_LIST_FILE" | tail -n +777 | cut -f 1 > "$WD"/output/url-links_folder_11_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 856 "$RAW_FILE_LIST_FILE" | tail -n +777 | cut -f 3 > "$WD"/output/url-links_folder_11_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 936 "$RAW_FILE_LIST_FILE" | tail -n +857 | cut -f 1 > "$WD"/output/url-links_folder_12_reads_1.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

head -n 936 "$RAW_FILE_LIST_FILE" | tail -n +857 | cut -f 3 > "$WD"/output/url-links_folder_12_reads_2.txt
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_01_reads_1.txt -P "$WD"/output/temp/folder_01/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 01"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_01_reads_2.txt -P "$WD"/output/temp/folder_01/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_02_reads_1.txt -P "$WD"/output/temp/folder_02/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 02"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_02_reads_2.txt -P "$WD"/output/temp/folder_02/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_03_reads_1.txt -P "$WD"/output/temp/folder_03/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 03"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_03_reads_2.txt -P "$WD"/output/temp/folder_03/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_04_reads_1.txt -P "$WD"/output/temp/folder_04/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 04"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_04_reads_2.txt -P "$WD"/output/temp/folder_04/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_05_reads_1.txt -P "$WD"/output/temp/folder_05/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 05"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_05_reads_2.txt -P "$WD"/output/temp/folder_05/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_06_reads_1.txt -P "$WD"/output/temp/folder_06/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 06"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_06_reads_2.txt -P "$WD"/output/temp/folder_06/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_07_reads_1.txt -P "$WD"/output/temp/folder_07/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 07"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_07_reads_2.txt -P "$WD"/output/temp/folder_07/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_08_reads_1.txt -P "$WD"/output/temp/folder_08/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 08"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_08_reads_2.txt -P "$WD"/output/temp/folder_08/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_09_reads_1.txt -P "$WD"/output/temp/folder_09/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 09"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_09_reads_2.txt -P "$WD"/output/temp/folder_09/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_10_reads_1.txt -P "$WD"/output/temp/folder_10/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 10"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_10_reads_2.txt -P "$WD"/output/temp/folder_10/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_11_reads_1.txt -P "$WD"/output/temp/folder_11/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 11"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_11_reads_2.txt -P "$WD"/output/temp/folder_11/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
wget --progress=dot:giga -i "$WD"/output/url-links_folder_12_reads_1.txt -P "$WD"/output/temp/folder_12/reads_1
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

echo "Correctly downloaded reads 1 from folder 12"
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi

wget --progress=dot:giga -i "$WD"/output/url-links_folder_12_reads_2.txt -P "$WD"/output/temp/folder_12/reads_2
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
