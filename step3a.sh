...
while read VCF_FILE; do
    ...
    grep ";SVTYPE=DEL" "$VCF_FILE" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_DEL_SV_lines.txt
    
    grep ";SVTYPE=INS" "$VCF_FILE" | grep -v ";REPTYPE=DUP" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_INS_SV_lines.txt
    
    grep ";SVTYPE=INS" "$VCF_FILE" | grep ";REPTYPE=DUP" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_DUP_SV_lines.txt
    ...
    while read DEL_SV_LINES_FILE; do
        ...
        paste "$FILE_COL_1" "$FILE_COL_2" "$FILE_COL_3" | awk '($3 - $2) >= 50' | sed 's/^/chr/g' > "$BED_FILE"
        if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
