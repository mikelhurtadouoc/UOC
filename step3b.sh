...
echo '**************************************************'
echo 'CONVERTING AND FILTERING VCF FILE INTO BED FILE ...'
date -R
...
while read DELLY_VCF_FILE; do
    ...
    grep ";SVTYPE=DEL" "$DELLY_VCF_FILE" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_DEL_SV_lines.txt
    
    grep ";SVTYPE=INS" "$DELLY_VCF_FILE" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_INS_SV_lines.txt
    
    grep ";SVTYPE=DUP" "$DELLY_VCF_FILE" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_DUP_SV_lines.txt
    ...
    while read DEL_SV_LINES_FILE; do
        ...
        paste "$FILE_COL_1" "$FILE_COL_2" "$FILE_COL_3" | awk '($3 - $2) >= 50' > "$BED_FILE"
        if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
...
while read WHAM_VCF_FILE; do
    ...
    grep ";SVTYPE=DEL" "$WHAM_VCF_FILE" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_DEL_SV_lines.txt
    
    grep ";SVTYPE=INS" "$WHAM_VCF_FILE" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_INS_SV_lines.txt
    
    grep ";SVTYPE=DUP" "$WHAM_VCF_FILE" | grep -v "##INFO" | awk -F'\t' '$7 == "PASS"' > "$WD"/output/temp/"$BED_FILE_FILENAME_NO_EXT"_DUP_SV_lines.txt
